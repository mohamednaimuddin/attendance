...
<head>
    ...
    :root {
        ...
        /* Secondary: Brighter Blue for primary actions (Add Log) */
        --action-button-hover: #2980b9;
        --accent: #f39c12;
        /* Sun Yellow/Orange for Warnings/Highlights (Filter) */
        --accent-hover: #e67e22;
        --success: #2ecc71;
        --error: #e74c3c;
        /* Red for Edited/Error */
        ...
        /* Light teal hover */
        --friday-highlight-light: #fffacd;
        /* Lemon Chiffon for light mode */
        --friday-highlight-text-light: #5a5a00;
    ...
    html.dark-mode {
        ...
        --action-button-hover: #2980b9;
        --accent: #f1c40f;
        /* Yellow/Gold for Warnings/Highlights */
        --accent-hover: #f39c12;
        --success: #2ecc71;
        --error: #ff6b6b;
        ...
        /* Light teal hover */
        --friday-highlight-dark: #3a3000;
        /* Darker yellow for dark mode */
        --friday-highlight-text-dark: #ffeeba;
        /* Lighter yellow text */
    }
    ...
    .btn-warning {
        ...
        color: var(--text-color);
        /* Dark text on light accent background */
    }
    ...
    .friday-row-highlight {
        ...
        /* Use main accent color for a strong left border */
    }
    ...
    <div class="container my-5">
        <div class="card p-4">
            ...
            <form method="get" class="row g-3 mb-3">
                <div class="col-md-3 col-sm-6">
                    ...
                    <input type="date" name="start" value="<?=htmlspecialchars($start)?>"
                        class="form-control form-control-sm" title="Start Date">
                </div>
                <div class="col-md-3 col-sm-6">
                    <label class="form-label visually-hidden">End Date</label>
                    <input type="date" name="end" value="<?=htmlspecialchars($end)?>"
                        class="form-control form-control-sm" title="End Date">
                </div>
                ...
                <div class="col-md-3 col-12 d-flex gap-1">
                    ...
                    <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal"
                        data-bs-target="#addMissingModal"><i class="fas fa-plus-circle"></i> Add Log</button>
                </div>
            ...
            <div class="report-info">
                <?php if($selected_user_name != 'All Users'): ?>EMPLOYEE:
                <span><?= htmlspecialchars($selected_user_name) ?></span><?php endif; ?>
                <?php if($selected_user_name != 'All Users' && $filtered_month_year_display): ?> | <?php endif; ?>
                <?php if($filtered_month_year_display): ?>PERIOD:
                <span><?= $filtered_month_year_display ?></span><?php endif; ?>
            </div>
            <?php endif; ?>
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">No attendance records found for the
                                selected period/user on this page.</td>
                        </tr>
                        <?php else: ?>
...
?>
                        <tr
                            class="log-summary-row <?= $anyEdited ? 'edited-row' : '' ?> <?= $friday_highlight_class ?>">
                            ...
                            <td><span
                                    class="badge <?= $val['overtime']>0?'badge-overtime':'bg-secondary' ?>"><?=fmt($val['overtime'])?></span>
                            </td>
                            <td><button class="btn btn-sm btn-info" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#collapse-<?=$log_uid?>-<?=$day_ts?>"><i class="fas fa-eye"></i>
                                    View</button></td>
                        </tr>
...
?>
                                            <tr class="sub-log-row"
                                                <?= $isEdited ? 'style="background-color: var(--error)10;"' : '' ?>>
                                                <td class="pt-2 pb-2">
                                                ...
                                                <td class="pt-2 pb-2">
                                                    <?php if($log['check_in_photo']): ?><a
                                                        href="uploads/<?=$log['check_in_photo']?>" target="_blank"><i
                                                            class="fas fa-image me-1"></i>In</a><?php endif; ?>
                                                    <?php if($log['check_out_photo']): ?><a
                                                        href="uploads/<?=$log['check_out_photo']?>" target="_blank"
                                                        class="ms-2"><i
                                                            class="fas fa-image me-1"></i>Out</a><?php endif; ?>
                                                </td>
                                                <td class="pt-2 pb-2">
                                                    <?php 
...
if($log['check_in_lat'] && $log['check_in_lng']): ?>
                                                    <a href="http://maps.google.com/maps?q=<?=$log['check_in_lat']?>,<?=$log['check_in_lng']?>"
                                                        target="_blank"><i class="fas fa-map-marker-alt me-1"></i>In</a>
                                                    <?php endif; ?>
                                                    <?php if($log['check_out_lat'] && $log['check_out_lng']): ?>
                                                    <a href="http://maps.google.com/maps?q=<?=$log['check_out_lat']?>,<?=$log['check_out_lng']?>"
                                                        target="_blank" class="ms-2"><i
                                                            class="fas fa-map-marker-alt me-1"></i>Out</a>
                                                    <?php endif; ?>
                                                </td>
                                                <td class="pt-2 pb-2">
                                                    <button class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                                        data-bs-target="#editModal<?=$log['id']?>"><i
                                                            class="fas fa-edit"></i> Edit</button>

                                                    <div class="modal fade" id="editModal<?=$log['id']?>" tabindex="-1"
                                                        aria-labelledby="editModalLabel<?=$log['id']?>"
                                                        aria-hidden="true">
                                                                ...
                                                                <form method="POST" action="update_log.php">
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title"
                                                                            id="editModalLabel<?=$log['id']?>">Edit Log
                                                                    ...
                                                                    <div class="modal-body">
                                                                        ...
                                                                        <div class="mb-3">
                                                                            ...
                                                                            <input type="datetime-local" name="check_in"
                                                                                value="<?=date('Y-m-d\TH:i', strtotime($log['check_in']))?>"
                                                                        ...
                                                                        <div class="mb-3">
                                                                            ...
                                                                            <input type="datetime-local"
                                                                                name="check_out"
                                                                                value="<?= $log['check_out'] ? date('Y-m-d\TH:i', strtotime($log['check_out'])) : '' ?>"
                                                                                class="form-control">
                                                                            <div class="form-text">Leave blank if the
                                                                                user hasn't checked out yet.</div>
                                                                        </div>
                                                                    </div>
        ...
        <a href="logout.php" class="btn logout-btn-footer">
            ...
            <div class="d-flex justify-content-between align-items-center mt-4">
                <small class="text-muted">
                    Showing logs <?= min($offset + 1, $total_records) ?> to <?= min($offset + $limit, $total_records) ?>
                ...
                <form id="addMissingForm" method="POST" action="add_missing_log.php">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="check_in_store" id="check_in_store" class="form-control"
                                placeholder="Store Name/Location" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="check_out_store" id="check_out_store" class="form-control"
